// features/visibility/hooks/useSetGroupVisibility.test.ts
import { act, renderHook, waitFor } from '@testing-library/react-native';
import { PostgrestError } from '@supabase/supabase-js';

import { supabase } from '../../../lib/supabase';
import { useSetGroupVisibility } from './useSetGroupVisibility';

jest.mock('../../../lib/supabase');

const mockSupabaseRpc = supabase.rpc as jest.MockedFunction<typeof supabase.rpc>;

describe('useSetGroupVisibility', () => {
  ...
  describe('setVisibility method', () => {
    ...
    it.each(actionCases)('calls RPC with correct params for %s action', async (action, expectedParams) => {
      mockSupabaseRpc.mockResolvedValue({ data: null, error: null } as any);

      const { result } = await renderHook(() => useSetGroupVisibility(mockRefetch));

      await act(async () => {
        await result.current.setVisibility('group-1', action as any);
      });

      expect(mockSupabaseRpc).toHaveBeenCalledWith('set_group_visibility', {
        p_group_id: 'group-1',
        ...expectedParams,
      });
    });
    ...
    it('returns { error: null } on success', async () => {
      ...
      await act(async () => {
        response = await result.current.setVisibility('group-1', '1h');
      });
    ...
    it('calls refetch on success', async () => {
      ...
      await act(async () => {
        await result.current.setVisibility('group-1', '1h');
      });
  ...
  describe('error message management', () => {
    ...
    it('clears error message on subsequent successful call', async () => {
      ...
      await act(async () => {
        await result.current.setVisibility('group-1', '2h');
      });
  ...
  describe('unmounted component handling', () => {
    it('does not update state after unmount', async () => {
      ...
      const { result, unmount } = await renderHook(() => useSetGroupVisibility(mockRefetch));
