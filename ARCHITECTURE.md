...
| FT-41 | **Redefines FT-22/FT-23's "journey" unit — day-grouping → trip-segmentation.** PO feedback (2026-09-04) after using FT-22/FT-23: a whole calendar day of raw fixes (walking office → kitchen → yard to water the lawn, all day, every day) is not a "journey" — a journey/trip should be one excursion: leaving a place you'd been stationary at, moving, arriving/settling at the next one (e.g. leave house → store → back home = one trip, not lumped into "today"). FT-22's locked "Journey definition" note explicitly chose day-grouping *over* trip-segmentation to avoid this exact heuristic ("FT-5's schema has no stop/start markers, so segment derivation would need a new, PO-unspecified heuristic... real scope creep for a list-only ticket") — PO is now asking for that heuristic. **Architect must design:** the stationary-gap heuristic itself (distance/time thresholds for "stopped" vs "moving," tuned so a stoplight/checkout line doesn't split one trip into two, but a morning errand and an evening walk don't merge into one), where it runs (client-derived from paginated raw rows, like `groupLocationHistoryByDay` today, vs. a new DB-side RPC), and how a trip is addressed/paginated (replacing `dateLocal` keying) for both the list and FT-23's playback. **Reuse assessment (already scoped, 2026-09-04, so the architect doesn't need to redo this triage):** throwaway — `groupLocationHistoryByDay.ts`+test (the day-bucketing algorithm being replaced), the `JourneyDay` type, and every `dateLocal`-keyed param/route (`useJourneyPlayback`, `PlaybackScreen`, `app/(app)/history/playback.tsx`) plus the RPC's day-range computation in `get_journey_playback_points`. Rework-not-rewrite — `useJourneyHistory.ts` (keyset pagination over raw rows stays, only the final grouping step changes), `JourneyList.tsx`/`HistoryScreen.tsx` (same shell, re-keyed from days to trips). Fully reusable, untouched — `is_hidden_from_group_at`/`is_globally_hidden_at`/the RPC's per-row redaction and coordinate-nulling (already timestamp-parameterized, indifferent to day vs. trip), `deriveRedactedWindows.ts`, `PlaybackMap.tsx`, `MemberSelector.tsx`, `useGroupRoster.ts`, `_layout.tsx`. FT-40 stays relevant regardless (it's about the RLS gate, not the grouping unit) — don't fold it into this ticket. | FT-22, FT-23 | ⬜ |
